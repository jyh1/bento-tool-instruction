...
```
We first use two `--request-*` options to specify the resources we would like to use. Important options inlcude: `rquest-gpus` for gpu number, `request-docker-image` for docker image and `request-memory` for memory size (default is 2g which is usually insufficient). Then we provide bundle dependency declarations. The canonical form of bundle dependency is `<name>:<UUID>` (You can use a shorthand form if referencing bundles from the current worksheet, which will be expanded to the canonical form by the program). The last part is the quoted command we want to run in the container.
